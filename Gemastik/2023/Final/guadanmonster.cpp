...
void dfs(int node, vector<vector<pair<int, bool>>> &adj, vector<int> &adjCount, vector<bool> &visited, queue<int> &q) {
    if(adjCount[node] != 0 || visited[node]) {
        return;
    }
    q.push(node);
    visited[node] = true;
    for(auto x : adj[node]) {
        adjCount[x.first]--;
        if(adjCount[x.first] == 0 && !visited[x.first]) {
            dfs(x.first, adj, adjCount, visited, q);
        }
    }
}
...
signed main() {
    ...
    queue<int> q;
    for(int i = 0; i < n; i++) {
        if(adjCount[i] == 0 && !visited[i]) {
            dfs(i, adj, adjCount, visited, q);
        }
    }
    /**
     * Note from Haiboko
